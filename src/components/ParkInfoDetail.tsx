...
const ParkInfoDetail = ({
	...
	return (
		<div
			className={clsx(
				'fixed z-10 px-10 pt-24 pb-10',
				backdrop && 'inset-0',
				!backdrop && 'top-0 left-0 h-full'
			)}
			onClick={() => backdrop && onClose()}
		>
			<div className='relative h-full w-md rounded-lg border-1 border-solid border-gray-100 bg-white p-5 shadow-xl'>
				<Close
					className='absolute top-5 right-5 h-6 w-6 text-black'
					onClick={() => onClose()}
				/>
